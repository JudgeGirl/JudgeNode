...
router.get('/download/:pid', function(req, res, next) {
	...
	dblink.problemManager.problemContent(pid, function(pcontent, pinfo, psubmit) {
		...
		dblink.problemManager.downloadList(pid, function(file_list) {
			res.render('layout', { layout: 'testdata_download', subtitle: 'Testdata', user: req.session,
									problemtitle: pid + '. ' + ttl, pid: pid, downloadList: file_list});
		});
	});
});
router.get('/manage', function(req, res, next) {
	...
	dblink.helper.isAdmin(uid, function(isadmin) {
		if (isadmin) {
			res.render('layout', { layout: 'testdata_manage', subtitle: 'Testdata', user: req.session });
		} else {
