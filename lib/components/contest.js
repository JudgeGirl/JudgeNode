...
module.exports = {
    ...
    scoreboard: async function(cid, uid, callback) {
        ...
        contest = contest[0];
        let startTime = contest.ts1;
        let endTime = contest.ts2;

        let table_config = {};
        table_config.cid = cid;

        let problems = await promise.scoreboard.getProblem(cid);
        table_config.header = problems;

        let stats = await promise.scoreboard.getUserScoreFromSubmission(cid);
        table_config.stats = stats || [];

        let userLookupTable = await promise.scoreboard.getUserInfoTable(cid);
        table_config.user = userLookupTable || [];

        table_config.scorekind = await promise.scoreboard.getRowScore(cid);

        let grade = await promise.scoreboard.getUserScore(cid);
        table_config.grade = grade || []; // for table of score disctribution
        ...
        table_config.icpc = {};
        table_config.icpc.ac = earliestACSubmission;
        table_config.icpc.wa = earliestNonACSubmission;
        table_config.icpc.contest = contest;
