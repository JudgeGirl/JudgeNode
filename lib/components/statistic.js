...
module.exports = {
	...
	best_submission: function(cid, pid, callback) {	// show top 20 submission
		var cmd = 'SELECT S.*, U.lgn FROM submissions S JOIN (SELECT sid, pid, uid, MIN(cpu) minVal FROM submissions WHERE pid = ? AND cid = ? AND res = 7 GROUP BY uid ) S2 ON S.cpu = S2.minVal AND S.res = 7 AND S.pid = S2.pid AND S.uid = S2.uid JOIN users U on U.uid = S.uid ORDER BY S.cpu, S.mem, S.len LIMIT 20';
		connection.query(cmd, [pid, cid], function(err, result) {
			console.log(err);
			callback(result);
		});
