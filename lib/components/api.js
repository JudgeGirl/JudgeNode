...
module.exports = {
	...
	problems: function(did, lid, uid, callback) {
		var cmd = 'SELECT pid, ttl FROM problems AS P JOIN level_domain AS L ON P.level = L.level AND L.did = ? AND P.level = ? WHERE pub != 0 ORDER BY P.level ASC, porder ASC';
		var ret = {};
		connection.query(cmd, [did, lid], function(err, result) {
			ret.plist = result;
			var cmd = 'SELECT S.pid, MAX(scr) as score FROM submissions AS S JOIN (SELECT pid FROM problems AS P JOIN level_domain AS L ON P.level = L.level AND L.did = ? AND P.level = ? WHERE pub != 0 ORDER BY P.level ASC, porder ASC) AS D ON D.pid = S.pid WHERE S.uid = ? GROUP BY uid, pid ORDER BY pid';
			connection.query(cmd, [did, lid, uid], function(err, result) {
				ret.score = result;
				var cmd = 'SELECT S.pid, COUNT(*) as count FROM submissions AS S JOIN (SELECT pid FROM problems AS P JOIN level_domain AS L ON P.level = L.level AND L.did = ? AND P.level = ? WHERE pub != 0 ORDER BY P.level ASC, porder ASC) AS D ON D.pid = S.pid GROUP BY pid ORDER BY pid';
				connection.query(cmd, [did, lid], function(err, result) {
					ret.subs = result;
					callback(ret);
				});
