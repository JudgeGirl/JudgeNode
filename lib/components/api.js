...
module.exports = {
    list: function(filter, isAdmin, isStrong, callback) {
        ...
        var cond, cmp, offset = 0, count = 20;
        var subLimit = [];
        cmp = 'S.sid DESC';
        ...
        var cmd = 'SELECT S.sid, S.ts, S.uid, S.pid, S.cid, S.res, S.scr, S.lng, S.cpu, S.mem, S.len, U.lgn, P.ttl, U.ip, 1 - ISNULL(RP.sid) as hasReport, G.name as guildTitle, G.icon as guildIcon, S.factor FROM ' +
    ...
    result: function(sid, isAdmin, callback) {
        ...
        var cmd = 'SELECT S.sid, S.ts, S.uid, S.pid, S.cid, S.res, S.scr, S.lng, S.cpu, S.mem, S.len, U.lgn, P.ttl, U.ip, 1 - ISNULL(RP.sid) as hasReport, S.factor FROM ' +
            '(SELECT S.sid, S.ts, S.uid, S.pid, S.cid, S.res, S.scr, S.lng, S.cpu, S.mem, S.len, S.factor FROM submissions S ORDER BY S.sid DESC LIMIT ?) S ' +
