...
module.exports = {
    ...
    result: function(sid, isAdmin, callback) {
        var cmd;
        if (!isAdmin)
            cmd = 'SELECT S.sid, S.ts, S.uid, S.pid, S.cid, S.res, S.scr, S.lng, S.cpu, S.mem, S.len, U.lgn, P.ttl, U.ip FROM submissions S, users U, problems P WHERE S.uid = U.uid AND S.pid = P.pid AND (P.pub = 1 OR U.class IS NOT NULL) ORAND S.sid = ? ORDER BY S.sid DESC LIMIT ?';
        else
