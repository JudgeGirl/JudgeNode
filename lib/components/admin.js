...
module.exports = {
    ...
    update_problem_content: async function(pconfig, callback) {
        ...
        let setList = ['pub', 'ttl', 'level', 'porder', 'ts', 'standard_cyclomatic_complexity'];
        setList = setList.map(col => col + ' = ?')
        let setStr = setList.join(', ');

        let cmd = [
            `UPDATE problems SET ${setStr}`,
            'WHERE pid = ?'
        ].join(' ');
        ...
        let parameterList = [
            ...
            pconfig.porder,
            new Date().getTime(),
            pconfig.cyclomatic_complexity,
            pid
        ];

        connection.query(cmd, parameterList, function(err, result) {
    ...
    create_contest_config: function(config, callback) {
        let cmd = 'INSERT INTO contests SET pub = ?, ttl = ?, ts1 = ?, ts2 = ?, sfrid = ?';
        let parameterList = [config.pub, config.ttl, config.ts1, config.ts2, config.style_factor_rule_id];
        connection.query(cmd, parameterList, function(err, result) {
            if (err)
                return callback();
    ...
    update_scores: function(uid, exam, callback) {
        ...
        for (var i in exam) {
            ...
            if (isNumeric(s))
                ...
                logger.warn(`invalid score: (${uid}, ${eid}, ${s})`);
            }
        }
        callback();
    },
