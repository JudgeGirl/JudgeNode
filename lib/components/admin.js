...
module.exports = {
    ...
    load_announcement: async function(callback) {
        try {
            let announcement = await announcementResource.getMD('announcement');
    ...
    update_announcement: async function(md_text, callback) {
        ...
        await announcementResource.setMD("announcement", md_text);
        await announcementResource.setHTML("announcement", html);
    ...
    load_problem_content: function(pid, callback) {
        let mdPath = `${config.RESOURCE.markdown.problem}/${pid}.md`;
        ...
        connection.query(cmd, [pid], function(err, result) {
            ...
            connection.query(cmd, [pid], function(err, result) {
                ...
                fs.readFile(mdPath, 'utf8', function(err, buf_data) {
                    if (err)
                        pconfig.md = '';
                    else
    ...
    update_problem_content: function(pconfig, callback) {
        ...
        let mdPath = `${config.RESOURCE.markdown.problem}/${pid}.md`;
        let htmlPath = `${config.RESOURCE.public.problem}/${pid}.html`;
        var html = markdown.post_marked(md_text);
        fs.writeFile(mdPath, md_text, function(err) {
