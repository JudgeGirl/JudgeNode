...
var sourceList = async function(pid, callback) {
    loggerFactory.getLogger(module.id).debug(`get source list: ${pid}`);
    let resource = resourceFactory.createSourceListResource();
    callback(await resource.get(pid));
};
...
var downloadList = async function(pid, callback) {
    ...
    try {
        ...
        loggerFactory.getLogger(module.id).silly(`Get source list: ${sourceListPath}.`, { result });
        callback(result);
    } catch (err) {
        loggerFactory.getLogger(module.id).verbose('Error in get download list.', { err });
        callback([]);
    }
};
...
var problemContent = async function(pid, callback) {
    let problemHtml;
    loggerFactory.getLogger(module.id).debug(`get problem content: ${pid}`);
    ...
    try {
        ...
        loggerFactory.getLogger(module.id).debug(err);
        callback(err);
        return;
    }
