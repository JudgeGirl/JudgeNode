...
var testdataList = function(pid, callback) {
    var source_list_path = config.JUDGE.testdata.path + "/" + pid + "/subtasks.py";
    fs.readFile(source_list_path, 'utf8', function(err, buf_data) {
        var testdata_list = [];
        if (!err)
...
var downloadList = async function(pid, callback) {
    let sourceListPath = config.HOST.resource + '/' + config.HOST.testdata + '/' + pid;
...
var problemContent = function(pid, callback) {
    var problemPath = config.JUDGE.path + 'source/problem/' + pid + '.html';
    fs.readFile(problemPath, 'utf8', function(err, problemHtmlStream) {
        var problemHtml = err ? 'Loading Error 1' : problemHtmlStream.toString();
...
var problemSolution = function(pid, callback) {
    var solutionPath = config.JUDGE.path + 'source/solution/' + pid + '.html';
    var sconfig = {};
    var cmd = 'SELECT * FROM problems WHERE pid = ?';
    connection.query(cmd, [pid], function(err, result) {
        ...
        fs.readFile(solutionPath, 'utf8', function(readSolutionError, buf_data) {
            if (readSolutionError) return callback(sconfig);
