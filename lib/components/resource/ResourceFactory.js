...
const FileTreeResource = require('./FileTreeResource');
...
class ResourceFactory {
    ...
    createSourceListResource() {
        ...
        cacheResource = new RedisCacheResource(cacheKeyFactory, 20);
    ...
    createDownloadListResource() {
        let cacheResource;
        let cacheKeyFactory;
        let fileTreeLocator;
        let fileTreeResource;

        let resource;
        fileTreeLocator = new TemplateFileLocator(`${config.RESOURCE.public.testdata}/{}`);
        fileTreeResource = new FileTreeResource(fileTreeLocator);
        fileTreeResource.setOutputConverter(new TransferNullConverter([]));
        fileTreeResource.setOutputConverter(
            new CompositeConverter(new TransferNullConverter([]), new FilterListConverter(['judge', 'special', 'subtasks.py', 'main.c', 'send.lst', 'source.lst']))
        );

        cacheKeyFactory = new TemplateCacheKeyFactory('download-list', 'testdata/{}');
        cacheResource = new RedisCacheResource(cacheKeyFactory, 20);
        cacheResource.setInputConverter(new ToJsonConverter());
        cacheResource.setOutputConverter(new FromJsonConverter());

        resource = new CachedResource(fileTreeResource, cacheResource);
