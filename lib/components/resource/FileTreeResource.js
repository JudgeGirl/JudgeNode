...
async function traverseDirectory(dir, level, skipLength) {
    let result = [];
    let fileList;
    ...
    for (const file of fileList) {
        ...
        if (await isDirectory(currentFile)) {
            let subDirTree = await traverseDirectory(currentFile, level - 1, skipLength);
        ...
        result.push(currentFile.substring(skipLength));
    }
...
class FileTreeResource extends ReadOnlyResource {
    ...
    getImplementation(key) {
        ...
        file = file.replace(/\/+$/, '');
        let skipLength = file.length + 1;

        moduleLogger.silly(`root = ${file}`);

        return traverseDirectory(file, this.maxLevel, skipLength);
    }
}
