...
module.exports = {
    info: function(uid, callback) { // show user page
        ...
        connection.query(cmd, [uid], function(err, result) {
            ...
            connection.query(cmd, [uid], function(err, result) {
                user.solve = result;
                var cmd = 'SELECT S.sid, S.ts, S.uid, S.pid, S.cid, S.res, S.scr, S.lng, S.cpu, S.mem, S.len, U.lgn, P.ttl, U.ip FROM submissions S, users U, problems P WHERE U.uid = ? AND S.uid = U.uid AND S.pid = P.pid ORDER BY sid DESC LIMIT 20';
                connection.query(cmd, [uid], function(err, result) {
                    user.recent = result;
                    callback(user);
                });
